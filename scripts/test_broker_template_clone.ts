...
import { renderBrokerSavingsClone } from "../src/templates/broker-savings-clone-renderer.ts";
...
const outDir = path.resolve("outputs/regression_broker_savings_clone");
const workspace = path.join(outDir, "template-clone/broker-savings");
...
const pptxPath = await renderBrokerSavingsClone({
  outDir,
