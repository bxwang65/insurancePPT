...
const outDir = path.resolve("outputs/regression_savings_ci_bundle_clone");
const workspace = path.join(outDir, "template-clone/savings-ci-bundle");
...
const req: PipelineRequest = {
  ...
  customerName: "测试客户",
  outputStem: "regression_savings_ci_bundle_clone",
...
const deck = await renderSavingsCiBundle({
  req,
  outDir,
