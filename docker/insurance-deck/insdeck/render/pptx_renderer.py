...
def render_pptx(data: Dict, output_path: str,
    ...
    if narrative_slides:
        ...
        # 2026-08-26: 法律屏障 slide (港险专属, 默认对所有储蓄险加, CI/IUL 不加)
        #   - 放在所有数据 slide 之后, 声明/来源之前, 作为"资产闭环"软内容营销页
        #   - 借鉴 AIA 财富盈活 PDF p3-4, 通用港险都适用
        #   - meta.include_legal_protection=False 可强制关闭 (override)
        if meta.get('has_savings') and meta.get('include_legal_protection', True):
            slides_def.append(('legal_protection', lambda: _slide_legal_protection(prs, meta, C)))
...
# ── 2026-08-26: 法律屏障 slide (借鉴 AIA 财富盈活 PDF p3-4) ────────────────
#   - 香港法 + 内地法双重保护, 5 道屏障覆盖夫妻/债务/信托/隐私
#   - 默认关闭 (meta.include_legal_protection 显式开启才显示)
#   - 仅港险适用 (planType='savings' 且 currency='USD'/'HKD'/'RMB')
#   - 文案来自 AIA 财富盈活 PDF + 香港法例条文, 销售端正式发布前需法务核实
_LEGAL_BARRIERS = [
    {
        'title': '香港夫妻分别财产制',
        'law': '《已婚者地位条例》第 182 章',
        'body': '香港实行夫妻分别财产制，与内地夫妻共同财产制不同。在香港缮发的保单，保单价值归保单持有人个人所有，不自动构成夫妻共同财产。',
    },
    {
        'title': '法定信托隔离',
        'law': '《已婚者地位条例》第 13 条',
        'body': '若人寿保单述明为受保人的妻子、丈夫或子女的利益而投保，保单须设立法定信托，根据该保单须支付的款项不得构成受保人的部分产业或受其债务所规限。',
    },
    {
        'title': '保单类信托功能',
        'law': '身故赔偿支付办法',
        'body': '香港保单本身具有类信托功能，身故赔偿支付办法允许保单持有人约定：一次性领取、分期领取、按年龄定期领取、增额领取等，无需额外设立信托或支付信托设立费用。',
    },
    {
        'title': '隐私保护',
        'law': '《个人资料（私隐）条例》第 486 章',
        'body': '香港有独立的私隐专员公署，金融信息未经授权不得查询或泄露。在香港缮发的保单受香港法律管辖，内地的法院、债权人或其他机构无法直接通过跨境渠道查询保单信息。',
    },
    {
        'title': '企业债务风险隔离',
        'law': '《公司法》第三条 + 《保险法》第 23/42 条',
        'body': '投保人/受保人合法持有的人寿保单现金价值，以及指定受益人的身故保险金，具备对抗第三方债权人非法追索的法律基础，公司债务原则上不及于个人保单。',
    },
]


def _slide_legal_protection(prs, meta, C):
    """2026-08-26: 法律屏障 slide (5 道法律保护)
    文案来自 AIA 财富盈活 PDF p3-4, 香港法 + 内地法双重保护
    """
    s = prs.slides.add_slide(prs.slide_layouts[6])
    fill_bg(s, C['bg_page'])
    add_header(s, meta=meta, C=C)
    add_title(s, '资产归属 · 五道法律屏障',
              '港险保单架构实现资产闭环保护 · 香港法 + 内地法双重适用',
              title_size=28, sub_size=11, C=C)

    # 顶部客户核心诉求框
    add_rect(s, Inches(0.5), Inches(2.0), Inches(12.3), Inches(0.85),
             fill='#F8E5E0', line=C['primary'], line_width=1.2)
    add_text(s, Inches(0.7), Inches(2.08), Inches(12.0), Inches(0.32),
             '【客户核心诉求】', size=11, color=C['primary'], bold=True)
    add_text(s, Inches(0.7), Inches(2.4), Inches(12.0), Inches(0.45),
             '无论未来企业经营风险、婚姻关系变化还是传承环节出现任何问题，这笔资产的归属权始终确定、不受外部追索。',
             size=10, color=C['dark_text'])

    # 5 张卡片 (2行: 3 上 2 下)
    cw = Inches(4.0); ch = Inches(1.65)
    gx = Inches(0.15); gy = Inches(0.15)
    sx = Inches(0.5); sy = Inches(3.05)
    for i, b in enumerate(_LEGAL_BARRIERS):
        col = i % 3; row = i // 3
        x = sx + col * (cw + gx); y = sy + row * (ch + gy)
        no_text = f'0{i+1}'
        add_rect(s, x, y, cw, ch, fill=C['bg_card'])
        add_rect(s, x, y, Inches(0.08), ch, fill=C['primary'])
        add_text(s, x + Inches(0.25), y + Inches(0.12), Inches(0.5), Inches(0.35),
                 no_text, size=14, color=C['primary'], bold=True)
        add_text(s, x + Inches(0.75), y + Inches(0.1), cw - Inches(0.9), Inches(0.35),
                 b['title'], size=12, color=C['dark_text'], bold=True)
        add_text(s, x + Inches(0.75), y + Inches(0.42), cw - Inches(0.9), Inches(0.28),
                 b['law'], size=8, color=C['primary'], bold=True)
        add_text(s, x + Inches(0.25), y + Inches(0.78), cw - Inches(0.4), ch - Inches(0.9),
                 b['body'], size=8.5, color=C['body_text'])

    # 底部小结 (橙色框, 呼应 AIA PDF)
    add_rect(s, Inches(0.5), Inches(6.85), Inches(12.3), Inches(0.45),
             fill='#FFF4E5', line='#E8A957', line_width=1.0)
    add_text(s, Inches(0.7), Inches(6.92), Inches(12.0), Inches(0.32),
             '五道法律屏障共同构成资产闭环，确保资金"雷打不动属于自己"。跨境资产保护以保费来源合法为前提。',
             size=10, color=C['dark_text'], bold=True)

    add_text(s, Inches(0.5), Inches(7.3), Inches(12.5), Inches(0.2),
             '注：本页法律条文内容仅供一般参考，正式签约前请咨询具有跨境执业资格的律师核实。',
             size=8, color=C['mid_text'])
