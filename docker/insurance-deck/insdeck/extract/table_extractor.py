                ...
                for ri, r in enumerate(t):
                    ...
                    for row_idx in range(max_rows if aligned else 1):
                        ...
                        if is_surrender and not is_withdraw:
                            ...
                            # Withdraw table: 该年提取款项 总额在 cells[4] (保 cells[2] + 非保 cells[3])
                            #                提取后退保价值 总额在 cells[8] (保 cells[6] + 非保 cells[7])
                            # 2026-09-12 bug fix: 之前只读 cells[2]/cells[3] 单列, 把"保证/非保证"子列当总额,
                            #                    导致 annual_withdrawal 偏小 (宏挚家 95575 → 应为 200000)
                            try:
                                annual_wd = int(cells[4][row_idx].replace(',','')) if row_idx < len(cells[4]) else 0
                            except: annual_wd = 0
                            if annual_wd == 0:  # 没有"总额"列的产品, fallback 到 A+B 求和
                                try:
                                    a = int(cells[2][row_idx].replace(',','')) if row_idx < len(cells[2]) else 0
                                    b = int(cells[3][row_idx].replace(',','')) if row_idx < len(cells[3]) else 0
                                    annual_wd = a + b
                                except: annual_wd = 0
                            try:
                                sv_after = int(cells[8][row_idx].replace(',','')) if row_idx < len(cells[8]) else 0
                            except: sv_after = 0
                            if sv_after == 0:
                                try:
                                    c = int(cells[6][row_idx].replace(',','')) if row_idx < len(cells[6]) else 0
                                    d = int(cells[7][row_idx].replace(',','')) if row_idx < len(cells[7]) else 0
                                    sv_after = c + d
                                except: sv_after = 0
