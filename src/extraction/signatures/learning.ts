...
async function inferPageTargets(pdfPath: string, planType: "savings" | "ci" | "iul"): Promise<PageTargets> {
  // 用 PyMuPDF 提取每页全文, 找出含 "退保总额" / "保障范围" / "保费表" 等关键词的页
...
for i, p in enumerate(doc):
    text = p.get_text()  # 全文而不仅是前 500 字符
    has_summary = "受保人" in text or "保單貨幣" in text or "保单货币" in text
    has_nowith = "退保總額" in text or "退保总额" in text or "現金價值" in text or "现金价值" in text
    has_wd = ("提取" in text) and ("每年" in text or "分紅" in text or "分红" in text)
    has_cov = "保障範圍" in text or "保障范围" in text or "保障項目" in text
