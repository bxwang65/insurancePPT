...
export function validateFormalIulPlan(plan: NormalizedIulPlan): FormalDeckIssue[] {
  ...
  if (!plan.indexAccounts.length) issues.push({ code: "IUL_INDEX_ACCOUNT_MISSING", level: "warn", message: "IUL 缺少指数账户配置 (部分 PDF 未列出指数账户明细, 默认 S&P 500)" });
  if (plan.benefitRows.length < 3) issues.push({ code: "IUL_BENEFIT_ROWS_INCOMPLETE", level: "error", message: `IUL 利益表仅提取 ${plan.benefitRows.length} 行` });
  if (plan.benefitRows.length >= 3 && plan.benefitRows.length < 20) issues.push({ code: "IUL_BENEFIT_ROWS_PARTIAL", level: "warn", message: `IUL 利益表仅提取 ${plan.benefitRows.length}/~50 行` });
