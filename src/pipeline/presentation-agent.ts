...
function getSavings(req: PipelineRequest): any | null {
  if (req.normalizedSavings) return req.normalizedSavings;
  // IUL 兼容: 将 IUL 标准化数据投影为 savings 字段, 复用 decadeRows / renderTable
  if (req.normalizedIul) {
    const i = req.normalizedIul as any;
    return {
      productName: i.productName,
      insured: i.insured,
      benefitRows: (i.benefitRows || []).map((r: any) => ({
        policyYear: r.policyYear,
        age: r.age,
        totalPremiumPaid: r.totalPremiumPaid,
        totalSurrenderValue: r.nonGuaranteedCashValue,
      })),
      withdrawalRows: [],
      // 用于缴费方式提示
      policy: i.policy,
...
function buildMarp(req: PipelineRequest, tenant: TenantBrandConfig, outline: OutlineArtifact, images: ImageArtifact, charts: ChartArtifact): string {
  ...
  const body = outline.slides.filter((s) => s.pageType !== "cover").map((s) => {
    ...
    if (s.pageType === "table") {
      ...
      // 缴费方式: 实际从 benefit illustration 算保费年数 (避免 policy 字段与真实数据不一致)
      const biAll: any[] = sdata?.benefitRows || [];
      const nonZeroPrem = biAll.filter((r: any) => Number(r?.annualPremium ?? r?.annual_premium ?? 0) > 0).length;
      const ap = Number(sdata?.policy?.annual_premium ?? sdata?.policy?.annualPremium ?? 0);
      const cur = String(sdata?.policy?.currency ?? "USD");
      const payLine = ap > 0
        ? `缴费方式：${cur} ${ap.toLocaleString("en-US", { maximumFractionDigits: 0 })}${nonZeroPrem > 0 ? " × " + nonZeroPrem + "年" : ""}`
        : `缴费方式：—`;
      return `---\n\n## ${esc(s.title)}\n\n<div class=\"table-full\">${renderTable(rows)}</div>\n<div class=\"metrics-bar\"><span>${esc(payLine)}</span><span>约第${d2 || "-"}年达到2倍</span><span>约第${d3 || "-"}年达到3倍</span><span>单利/复利用于观察阶段性效率</span></div>\n`;
    }
