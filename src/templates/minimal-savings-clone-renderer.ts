...
export interface MinimalSavingsCloneRequest {
  outDir: string;
...
export async function renderMinimalSavingsClone(req: MinimalSavingsCloneRequest): Promise<string> {
  const workspace = path.join(req.outDir, "template-clone", "minimal-savings");
  ...
  await run(process.env.PRESENTATIONS_PYTHON || DEFAULT_PYTHON, [
    path.join(ROOT, "scripts/build_minimal_savings_clone_plan.py"),
