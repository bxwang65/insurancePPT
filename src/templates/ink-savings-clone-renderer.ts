...
export interface InkSavingsCloneRequest {
  outDir: string;
...
export async function renderInkSavingsClone(req: InkSavingsCloneRequest): Promise<string> {
  const workspace = path.join(req.outDir, "template-clone", "ink-savings");
  ...
  await run(process.env.PRESENTATIONS_PYTHON || DEFAULT_PYTHON, [
    path.join(ROOT, "scripts/build_ink_savings_clone_plan.py"),
