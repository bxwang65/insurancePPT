...
export interface BrokerSavingsCloneRequest {
  outDir: string;
...
export async function renderBrokerSavingsClone(req: BrokerSavingsCloneRequest): Promise<string> {
  const workspace = path.join(req.outDir, "template-clone", "broker-savings");
  ...
  await run(process.env.PRESENTATIONS_PYTHON || DEFAULT_PYTHON, [
    path.join(ROOT, "scripts/build_broker_savings_clone_plan.py"),
